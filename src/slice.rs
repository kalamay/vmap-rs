...
/// Slice represents Span types that can also be split in two. This allows
/// the various slice types (RefSlice, ArcSlice, etc.) to be used generically.
pub trait Slice: Span + Sized {
...
/// use vmap::{Map, RefSlice};
///
/// # fn main() -> std::io::Result<()> {
/// let map = Map::open("README.md")?;
/// let slice = RefSlice::new(&map, 113..143);
...
/// # }
/// ```
pub struct RefSlice<'a, M> {
    map: &'a M,
    rng: ByteRange,
}

impl<'a, M> RefSlice<'a, M>
where
    M: Span,
{
...
impl<'a, M> Slice for RefSlice<'a, M>
where
    M: Span,
{
    fn split_at(self, mid: usize) -> (Self, Self) {
...
impl<M> SpanMut for RefSlice<'_, M>
where
    M: SpanMut,
{
...
impl<M> DerefMut for RefSlice<'_, M>
where
    M: SpanMut,
{
    #[inline]
...
impl<M> AsRef<[u8]> for RefSlice<'_, M>
where
    M: Span,
{
    #[inline]
...
impl<M> AsMut<[u8]> for RefSlice<'_, M>
where
    M: SpanMut,
{
    #[inline]
...
impl<'a, M> From<&'a M> for RefSlice<'a, M>
where
    M: Span,
{
...
impl<M> fmt::Debug for RefSlice<'_, M>
where
    M: Span,
{
...
mod tests {
    ...
    fn test_slice() {
        ...
        let mut slice = RefSlice::from(&map);
        slice[0] = 88;
        slice[10] = 99;
